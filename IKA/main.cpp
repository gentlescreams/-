...
int main(int argc, char *argv[]){
	...
	/*プレイヤーチームの作成*/
	//player1
	Ship player = Ship(glm::vec3(0, 0.5, 0),TEAM_PLAYER,TEAM_PLAYER_SHIP, PLAYER_1);
	ships.push_back(player);
	Ship player2 = Ship(glm::vec3(FIELD_SIZE, 0.5, 0), TEAM_PLAYER,TEAM_PLAYER_SHIP, OTHER);//glm::vec3& _pos, int _team, int _shipColor, int _playerID
	ships.push_back(player2);


	/*敵チームの生成*/
	//player2
	Ship enemy = Ship(glm::vec3(0, 0.5, FIELD_SIZE), TEAM_ENEMY,TEAM_ENEMY_SHIP, PLAYER_2);
	ships.push_back(enemy);
	Ship enemy2 = Ship(glm::vec3(FIELD_SIZE, 0.5, FIELD_SIZE), TEAM_ENEMY, TEAM_ENEMY_SHIP, OTHER);
	...
	//モデル読み込み
	model.ModelDataLoad("ikachan4.blend.x");
	for (Ship& sp : ships) {
		model.Bindtexture(sp.team);
	}
	/*加速度の初期化*/

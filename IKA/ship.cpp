...
Ship::Ship(glm::vec3& _pos, int _team, int _bodyColor ,int _playerID ){
	pos = _pos;
...
void Ship::Control(float _x, float _y, unsigned int _downKeys,unsigned int _pressedKeys){
	...
	if ((_y <0.3) && (_y >-0.3))//微妙な数値が入ったときの処理
	{
		acc = glm::vec3(0,0,0);
	}
	speed += acc / 60.0f;
	...
	if ((_x <0.3) && (_x >-0.3))//微妙な数値が入ったときの処理
	{
		YawAcc = 0;
